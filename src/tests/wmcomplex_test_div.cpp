// wmcomplex -- wmcomplex_test_div.cpp
// Author: Will Marshall
// Created: 4/15/2026, 7:53:53 PM
...
static const wm::TestLogger logger("wmcomplex_test_div");
...
inline std::pair<bool, std::string> test_logic() noexcept {
  ...
  try {
    const auto re_1 = 123.3f;
    const auto re_2 = 456.6;
    const auto im_1 = 532.3;
    ...
    const double expected_real = (((re_1 * re_2) + (im_1 * im_2)) / ((re_2*re_2) + (im_2 * im_2)));
    const double expected_imag = (((im_1 * re_2) - (re_1 * im_2)) / ((re_2*re_2) + (im_2 * im_2)));

    if (wm::epsilon_neq(complex_1.creal(), expected_real)) {
      ret_str
        .append("Observed an incorrect value, expected ")
        .append(std::to_string(expected_real))
        .append(", but observed ")
        .append(std::to_string(complex_1.creal()));
      ret_bool = false;
    } else if (wm::epsilon_neq(complex_1.cimag(), expected_imag)) {
      ret_str
        .append("Observed an incorrect value, expected ")
        .append(std::to_string(expected_imag))
        .append(", but observed ")
...
int main() {
  const std::string TEST_FRIENDLY_NAME = "complex division";
